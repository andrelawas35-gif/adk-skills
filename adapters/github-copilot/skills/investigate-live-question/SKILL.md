...
description: "investigate-live-question — GitHub Copilot adapter"
...
`conduct-work-object` to frame the Inquiry; do not substitute a broad research
topic.
...
- **prototype-ready:** transition the Inquiry to `design`, add a
  ...
  `next_action` to `design-tracer-bullet`. If a separate project Work Object
...
- **answered:** route to `conduct-work-object` with the bounded answer and
  evidence ledger.
- **reframed:** route to the conductor with the replacement question and why
  the original no longer governs.
- **prototype-ready:** route to `design-tracer-bullet` with the uncertainty a

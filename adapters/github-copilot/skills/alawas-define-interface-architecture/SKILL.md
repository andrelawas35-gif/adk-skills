...
- Define individual screen layouts or component specifications
  (that is `alawas-define-interface-specification`).
...
**Preconditions:** `alawas-model-user-flow` has produced flow definitions. The
...
Apply the `alawas-define-interface-architecture` profile and continuous Grilling
...
**Route when:**
- Architecture is defined — return to `alawas-conduct-work-object`.
- Architecture decisions require pressure testing — route to
  `alawas-pressure-test-decision`.

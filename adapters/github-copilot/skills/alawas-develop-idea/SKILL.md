...
- Converge on a single answer — that is `alawas-investigate-live-question`'s job
- Challenge decisions adversarially — that is `alawas-pressure-test-decision`'s job
- Run a full grilling architecture session — that is `alawas-grilling-session`'s job
...
- `alawas-conduct-work-object` has already established the workspace and Work Object
...
Apply the `alawas-develop-idea` profile and continuous Grilling Session in
...
`type: inquiry`. Either `alawas-conduct-work-object` routes here, or the user
invokes directly.
...
4. Route to `alawas-conduct-work-object` for next-stage routing.

Do NOT implement the selected direction. Implementation is routed through
`alawas-design-tracer-bullet` and `alawas-implement-bounded-change`.
...
**Route to `alawas-conduct-work-object` when:**
...
**Route to `alawas-investigate-live-question` when:**
...
**Route to `alawas-design-tracer-bullet` when:**

...
description: "diagnose-production-incident — GitHub Copilot adapter"
...
- **Affected path restored and verified:** route to `conduct-work-object` for `observe` with a bounded monitoring window; do not claim closure.
...
- **Prevention accepted:** route the linked successor to `implement-bounded-change` only after its own acceptance and authority.

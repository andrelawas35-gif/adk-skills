---
name: alawas-audit-product-interface
...
- Discover design tokens (that is `alawas-build-design-foundation`'s domain).
...
Apply the `alawas-audit-product-interface` profile and continuous Grilling Session
...
**Route when:**
- Discovery is complete — return to `alawas-conduct-work-object` with the snapshot.

...
description: "govern-scorecards — GitHub Copilot adapter"
...
- A proposed rule remains an unconfirmed Workflow Candidate until the existing
  ...
  in `maintain-working-method` are satisfied.
...
`pressure-test-decision` before recording it.

## Skill-aware grilling lens

Apply the `govern-scorecards` lens in
...
`maintain-working-method`; do not apply the proposal or treat one scorecard as
universal proof.
...
- Route a workflow-rule proposal, revision, contrary-evidence review, bounded
  test, promotion, or retirement to `maintain-working-method`.
- Route unresolved outcome evidence to `review-outcome-and-adapt` or
  `investigate-live-question`.
- Route a material governance decision to `pressure-test-decision`.
- Report a missing dependency as reduced capability; do not imitate its gates.

## Work Object updates

Return a concise record to `conduct-work-object` containing the scorecard
...
- **Evidence supports a proposal:** route the unconfirmed scoped candidate to
  `maintain-working-method`; do not promote or apply it.
...
- **Material revision:** route the linked successor through
  `maintain-working-method` using `supersedes`; do not mutate the predecessor.

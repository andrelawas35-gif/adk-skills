---
name: alawas-verify-design-implementation
...
- Verify that the agent's implementation matches the confirmed proposal
  from `alawas-apply-design-direction`.
...
- A confirmed proposal from `alawas-apply-design-direction` (the `[system:design-direction]`
  evidence entry).
...
Apply the `alawas-verify-design-implementation` profile and continuous Grilling
...
`alawas-apply-design-direction`. Extract the confirmed interpretation: what was
...
**Route when:**
- Verification report is complete — return to `alawas-conduct-work-object`.
- Corrections needed — route to `alawas-apply-design-direction` with the specific
  divergences.

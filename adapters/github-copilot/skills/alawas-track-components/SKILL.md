...
record the explicit gap and route to `alawas-conduct-work-object`; do not guess it.
...
- `file_write` — return a bounded ledger or inbox mutation to
  `alawas-conduct-work-object` for persistence.
...
Apply the `alawas-track-components` profile and continuous Grilling Session in
...
reasoning `[inference]`. Return `alawas-conduct-work-object` a concise record with

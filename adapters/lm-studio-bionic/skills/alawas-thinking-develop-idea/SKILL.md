---
name: alawas-thinking-develop-idea
...
- Converge on a single answer — that is `alawas-research-investigate-live-question`'s job
- Challenge decisions adversarially — that is `alawas-thinking-pressure-test-decision`'s job
- Run a full grilling architecture session — that is `alawas-thinking-grilling-session`'s job
...
- `alawas-governance-conduct-work-object` has already established the workspace and Work Object
...
Apply the `alawas-thinking-develop-idea` profile and continuous Grilling Session in
...
`type: inquiry`. Either `alawas-governance-conduct-work-object` routes here, or the user
invokes directly.
...
4. Route to `alawas-governance-conduct-work-object` for next-stage routing.

Do NOT implement the selected direction. Implementation is routed through
`alawas-design-design-tracer-bullet` and `alawas-engineering-implement-bounded-change`.
...
mutating command) per `alawas-governance-conduct-work-object`'s CLI-only write
...
**Route to `alawas-governance-conduct-work-object` when:**
...
**Route to `alawas-research-investigate-live-question` when:**
...
**Route to `alawas-design-design-tracer-bullet` when:**
...
| `web_fetch` | `Web access (native document project)` | native |

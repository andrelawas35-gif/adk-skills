---
name: alawas-operations-diagnose-production-incident
...
Apply the `alawas-operations-diagnose-production-incident` profile and continuous Grilling
...
- **Affected path restored and verified:** route to `alawas-governance-conduct-work-object` for `observe` with a bounded monitoring window; do not claim closure.
...
- **Prevention accepted:** route the linked successor to `alawas-engineering-implement-bounded-change` only after its own acceptance and authority.

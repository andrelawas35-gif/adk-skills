---
name: alawas-operations-deploy-with-recovery
...
Apply the `alawas-operations-deploy-with-recovery` profile and continuous Grilling Session in
...
- **Increment verified:** route to `alawas-governance-conduct-work-object` for `observe` with
...
| `file_uploads` | `Document/file attach (native in-project); external deployment upload via user instruction` | manual-fallback |
...
- **Best-effort tool**: `Document/file attach (native in-project); external deployment upload via user instruction`
- **Behavior**: Pause and give one concrete manual instruction.
- **Record**: Append History entry noting the capability gap, the
  manual action taken, and what remains unverified.
- **Note**: Bionic can attach documents and files within a project, but uploading deployment artifacts to an external target platform is not a documented native capability. Give the user the exact upload instruction.

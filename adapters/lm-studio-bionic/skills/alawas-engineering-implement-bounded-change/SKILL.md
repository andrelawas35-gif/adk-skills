...
| `background_processes` | `Shell tool (background) / background sessions` | manual-fallback |
...
- **Best-effort tool**: `Shell tool (background) / background sessions`
...
- **Note**: Bionic supports background sessions, but persistent daemon processes are not a documented first-class capability. Start long-running services manually or verify they remain running.

---
name: alawas-research-investigate-live-question
...
`alawas-governance-conduct-work-object` to frame the Inquiry; do not substitute a broad research
topic.
...
Apply the `alawas-research-investigate-live-question` profile and continuous Grilling Session
...
- **prototype-ready:** transition the Inquiry to `design`, add a
  ...
  `next_action` to `alawas-design-design-tracer-bullet`. If a separate project Work Object
...
- **answered:** route to `alawas-governance-conduct-work-object` with the bounded answer and
  evidence ledger.
- **reframed:** route to the conductor with the replacement question and why
  the original no longer governs.
- **prototype-ready:** route to `alawas-design-design-tracer-bullet` with the uncertainty a
...
- **Note**: Live web search is not a documented native Bionic capability. The agent can fetch known URLs but cannot perform open-ended web searches without a configured tool.

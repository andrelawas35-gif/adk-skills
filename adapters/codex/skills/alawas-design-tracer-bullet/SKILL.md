...
description: "alawas-design-tracer-bullet — Codex (VS Code) adapter"
platform: codex
---
...
- Record only an accepted design and route it to `alawas-conduct-work-object` for
  the next specialist.
...
**Preconditions:** `alawas-conduct-work-object` has discovered the workspace and
...
route to `alawas-conduct-work-object` to append a concise Decision and History entry.
...
Route the accepted design to `alawas-implement-bounded-change` (when available) or
back to `alawas-conduct-work-object` with the recommended next specialist. State that
...
- **Accepted design:** route through `alawas-conduct-work-object` to the next

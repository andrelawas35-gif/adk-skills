"""Contract tests for the user flow modeling skill."""
...
CORE = ROOT / "skills" / "core" / "model-user-flow" / "SKILL.md"
PLATFORMS = ("codex", "claude-code", "github-copilot")


class ModelUserFlowContract(unittest.TestCase):
    def test_core_skill_maps_user_goals_to_flows(self):
        text = CORE.read_text()
    ...
    def test_generated_adapters_preserve_core_and_include_references(self):
        ...
        for platform in PLATFORMS:
            with self.subTest(platform=platform):
                adapter_dir = ROOT / "adapters" / platform / "skills" / "alawas-model-user-flow"

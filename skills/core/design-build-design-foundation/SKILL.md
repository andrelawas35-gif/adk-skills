...
- Produce a `[system:token-inventory]` Evidence Ledger entry containing the
  token snapshot.
- Operate independently of `audit-product-interface` — token discovery may
  run without structural discovery (DEC-6).

This skill does not:

- It does not modify any host-project files, configuration, or tokens.
- Discover routes, components, or layouts (that is `audit-product-interface`).
...
Apply the `build-design-foundation` profile and continuous Grilling Session
...
**Route when:**
- Token discovery is complete — return to `conduct-work-object`.

...
- `file_write` — return a bounded ledger or inbox mutation to
  `conduct-work-object` for persistence.
...
Apply the `track-components` profile and continuous Grilling Session in
...
reasoning `[inference]`. Return `conduct-work-object` a concise record with

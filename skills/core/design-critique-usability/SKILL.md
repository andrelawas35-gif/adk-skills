...
`verify-design-implementation` answers one question: does the code match
...
creative decision — a finding is a candidate for `apply-design-direction`
...
- Explicitly name which heuristics were *not* checked in a given pass and
  ...
  test), matching `verify-design-implementation`'s own honesty about
  deferred dimensions.
...
- Run independent of whether a specific direction has been confirmed —
  ...
  this skill does not wait on `apply-design-direction`'s cycle.
...
- Check parity against a confirmed proposal — that remains
  `verify-design-implementation`'s job; this skill evaluates the design's
...
`design-audit-accessibility` and `build-design-foundation` already claim:
...
- `browser_automation` — inspect a live running application when the
  ...
  confirms, same posture as `verify-design-implementation` and
  `audit-accessibility`).
...
**Route when:**
- The critique report is complete — return to `conduct-work-object`.

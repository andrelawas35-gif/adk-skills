...
- Define screen layouts, component hierarchies, or navigation structure
  (that is `define-interface-architecture`).
...
`audit-product-interface`. The host project root is identifiable.
...
Apply the `model-user-flow` profile and continuous Grilling Session in
...
**Route when:**
- Flow definition is written — return to `conduct-work-object`.
- Gaps require design decisions — route to `define-interface-architecture`.

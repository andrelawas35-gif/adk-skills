...
uncertainty*: it composes `investigate-live-question`'s question-answering
...
- Decompose a broad research request into falsifiable sub-questions and
  invoke `investigate-live-question` once per sub-question (directly, or via
...
- Answer a sub-question itself — that discipline belongs entirely to
  `investigate-live-question`; this skill composes it, never duplicates or
...
- Export, publish, or write the deliverable anywhere outside
  ...
  `governance-conduct-work-object`'s authority rules already require for any write
  outside that boundary.
- Contact people, production, or sensitive sources — any sub-question
  needing that stays inside `investigate-live-question`'s own authority
...
**Preconditions:**
- `governance-conduct-work-object` has established the Work Object.
- For report-type work, `research-investigate-live-question` is available to invoke
...
- `subagent_spawn` — parallelize independent sub-question investigations
  (optional; sequential invocation of `investigate-live-question` is always
  valid).
...
- Writing the deliverable, or a copy of it, anywhere outside
  ...
  `governance-conduct-work-object` requires for any write outside that boundary. This
...
- A plan-type deliverable that would need to author a new decision (not
  ...
  and route to `thinking-pressure-test-decision` rather than writing it in.
...
belongs to `investigate-live-question`). Nominate a Candidate Card only when
...
Apply the `research-produce-report` profile and continuous Grilling Session in
...
- **Report-type:** decompose the request into distinct falsifiable
  sub-questions. Invoke `investigate-live-question` once per sub-question
...
- **Plan-type:** read the source Work Object(s)' `Decisions and revisit
  triggers` and `History` in full. Do not invoke `investigate-live-question`
...
to `governance-conduct-work-object` for durable recording. An unresolved
...
- **Deliverable produced:** route to `governance-conduct-work-object` with the path
...
- **A sub-question came back unresolved:** report it honestly inside the
  deliverable; route to `research-investigate-live-question` again only if the director
  wants that specific gap closed before finalizing.
- **A plan-type request needs a new decision, not a synthesis:** stop before
  authoring it in. Route to `thinking-pressure-test-decision`.
...
- Answering a sub-question directly instead of invoking
  `investigate-live-question` — bypasses its evidence discipline.

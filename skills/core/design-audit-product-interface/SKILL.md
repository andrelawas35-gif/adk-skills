...
- Discover design tokens (that is `build-design-foundation`'s domain).
...
Apply the `audit-product-interface` profile and continuous Grilling Session
...
**Route when:**
- Discovery is complete — return to `conduct-work-object` with the snapshot.

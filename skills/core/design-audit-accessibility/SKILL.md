...
testing it: `steward-experience-patterns` stewards accessibility as a
*written expectation*, and disclaims any compliance claim from that writing
alone; `verify-design-implementation` marks WCAG, keyboard, and
...
only `apply-design-direction` and its implementation/verification chain
...
- Compare findings against a stewarded `[system:pattern]` accessibility
  expectation from `steward-experience-patterns` when one exists for the
...
- Explicitly defer, not silently omit, keyboard-navigation, focus-order, and
  ...
  a reason, matching `verify-design-implementation`'s own honesty pattern.

This skill does not:

- Fix, patch, or otherwise mutate any code, style, or markup — findings
  route to `apply-design-direction` as candidate direction changes.
...
- Author or approve an accessibility-expectations pattern — that remains
  `steward-experience-patterns`'s domain; this skill only tests against
...
same independence `build-design-foundation` already claims from structural
discovery.
...
**Route when:**
- The audit report is complete — return to `conduct-work-object`.
- A finding should be fixed — route to `apply-design-direction` with the
...
- No stewarded expectation exists for the surface and the director wants
  one — route to `steward-experience-patterns` to author it, then re-audit

...
- Render interfaces (that is `render-to-figma`).
- Discover components (that is `audit-product-interface`).
...
**Preconditions:** `audit-product-interface` has produced a discovery snapshot.
...
Apply the `connect-design-to-code` profile and continuous Grilling Session
...
**Route when:**
- Registry is updated — return to `conduct-work-object`.

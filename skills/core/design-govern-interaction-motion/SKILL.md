---
name: govern-interaction-motion
default_tier: high
...
`build-design-foundation` discovers animation and transition tokens that
already exist in code; `verify-design-implementation` checks whether a
...
— as a governed asset record, the same discipline `compose-design-system`
...
- Check whether an interaction fires correctly in a running application;
  route to `design-verify-design-implementation`.
...
trigger to `conduct-work-object`.

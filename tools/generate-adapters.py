...
PLATFORMS = ["codex", "claude-code", "github-copilot", "lm-studio-bionic", "opencode", "opencode"]
...
# WO 2026-08-25-005 tracer: prove one shared-reference loading path without
# changing the canonical skill body. The generated adapter still has the same
# relative reference path, but that file is now a small resolver pointer to the
# repository-level canonical reference instead of a duplicated full copy.
SHARED_REFERENCE_POINTER_PILOTS = {
    ("thinking-pressure-test-decision", "CONSEQUENCE-AUTHORITY.md"),
    ("governance-conduct-work-object", "CONSEQUENCE-AUTHORITY.md"),
    ("governance-conduct-work-object", "AGREEMENT-LOOP.md"),
    ("governance-conduct-work-object", "SKILL-AWARE-GRILLING.md"),
    ("governance-conduct-work-object", "EVIDENCE-MODEL.md"),
    ("governance-conduct-work-object", "DIRECTOR-LANGUAGE.md"),
    ("governance-conduct-work-object", "MISSING-ARTIFACT-GAP.md"),
}
...
def uses_shared_reference_pointer(core_skill_name: str, filename: str) -> bool:
    """True when this skill/reference pair uses the shared-reference pilot."""
    return (core_skill_name, Path(filename).name) in SHARED_REFERENCE_POINTER_PILOTS


def build_shared_reference_pointer(source: Path, destination: Path) -> bytes:
    """Build a deterministic resolver file for a shared canonical reference."""
    rel = os.path.relpath(source, start=destination.parent).replace(os.sep, "/")
    text = (
        "# Shared Reference Pointer\n\n"
        "<!-- Generated by tools/generate-adapters.py for WO 2026-08-25-005. "
        "Do not hand-edit; edit the canonical source and regenerate. -->\n\n"
        f"Canonical source: `{rel}`\n\n"
        "This generated adapter file intentionally does not duplicate the "
        "canonical reference body. When a skill instruction asks for this "
        "relative `references/` file, load the canonical source above and "
        "treat this file as a resolver pointer.\n"
    )
...
def generate_adapter_section(overlay, required, default_tier="medium"):
    ...
    lines.append("")
    lines.append("For a high-consequence Work Object, confirmation must name the exact")
    lines.append("proposed mutation. Do not stage, annotate, change status, append History,")
    lines.append("or make any other mutation before receiving that scoped confirmation.")
    lines.append("")
...
def build_reference_entries(skill_name, output_dir, core_skill_dir=None, write=False):
    ...
    for filename in active_refs:
        ...
        if core_skill_dir is not None and uses_shared_reference_pointer(core_skill_dir.name, filename):
            content = build_shared_reference_pointer(source, destination)
        else:
            content = source.read_bytes()
        if write:
            if uses_shared_reference_pointer(core_skill_dir.name if core_skill_dir else "", filename):
...
CORE_ASSEMBLY_ORDER = [
    ...
    ("Evidence rules", ("whole", "Evidence rules")),
...
def assemble_core_from_contract(skill_dir):
    ...
    for heading, resolution in CORE_ASSEMBLY_ORDER:
        content = top_blocks.get(heading)
        if content is None:

...
|---|---|---|---|
| `business-manage-liquidity-and-cash-runway` | Produces bounded liquidity/escalation decisions (hold a cash buffer, trigger escalation, accept a gap) whose downstream cash outcome is directly observable against a dated threshold. | Emit a `ws relation add` edge (`observes` when the decision accepts a considered option, `hands_off_to` when it routes an escalation to another owner's Work Object) from the deciding Work Object using the now-real tracer tooling from WO `2026-08-22-026`. Since `ws decision`/`verification`/`outcome` commands remain unbuilt (section 1, "Important non-capabilities"), the Work Object's own structured `## Decisions` record stands in for a `DEC-*` for now — no new tooling beyond `ws relation add` is required. When the runway assumption is later checked against actual cash position, add a second `observes` edge back to close the loop (Direction 2, not yet built). | **Amend now — pilot business skill; first business-family entry, chosen for its directly observable outcome (cash vs. threshold).** |
